...
extern sint32 gSelectedZayBoxID;
...
ZEZayBox::ChildType ZEZayBox::GetChildType() const
{
...
{
    Point NewPos;
    mDraggingEditorPos[type].DeliveryOne(idx, ToReference(NewPos));
}

Strings ZEZayBox::SubInput(InputType type, sint32 idx, bool copyonly)
{
    if(!copyonly)
        mDraggingEditorPos[type].SubtractionSection(idx);
    return Strings();
}
...
void ZEZayBox::RenderTitle(ZayPanel& panel, chars title, ChildType childtype, bool stable, bool copy, bool expand, bool resize, bool remove)
{
    ...
    ZAY_LTRB_UI(panel, 0, 2, panel.w() - 4, TitleBarHeight - 2, UITitle,
        ZAY_GESTURE_VNTXY(v, n, t, x, y, this)
        {
            ...
            {
                if(!WasDragged)
                    Platform::SendNotify(v->view(), "ZayBoxSelect", sint32o(mID));
                if(mParent != -1)
    ...
    if(stable)
    {
        ...
        ZAY_XYWH_UI(panel, 5, -25, 55, 20, UIStatle,
        ...
        {
            ZAY_RGBA(panel, 128, 128, 128, (mStableMode)? 128 : ((IsStableFocusing)? 80 : 40))
                panel.ninepatch(R("box_slot_inner_select"));
            ZAY_FONT(panel, 1.0, gTitleFont)
            ZAY_RGBA(panel, 222, 252, 11, (mStableMode)? 255 : ((IsStableFocusing)? 120 : 40))
                panel.text("stable");
        }
    }
    ...
    if(IsSelected())
    {
            ...
            ZAY_XYWH_UI(panel, panel.w() - 145, -25, 55, 20, UICopy,
                ZAY_GESTURE_T(t, this)
                {
                    if(t == GT_InReleased || t == GT_OutReleased)
                    {
                        ...
                        if(0 < children().Count())
                            ZEZayBox::Save(children(), Json.At("ui"), false);
                        Platform::Utility::SendToTextClipboard(Json.SaveJson());
                    }
                })
            ZAY_MOVE_IF(panel, 1, 1, IsCopyPressed)
            {
                ZAY_RGBA(panel, 128, 128, 128, (IsCopyFocusing)? 128 : 80)
                    panel.ninepatch(R("box_slot_inner_select"));
                ZAY_FONT(panel, 1.0, gTitleFont)
                ZAY_RGBA(panel, 222, 252, 11, (IsCopyFocusing)? 255 : 120)
                    panel.text("copy");
            }
        }
        ...
        ZAY_XYWH_UI(panel, panel.w() - 85, -25, 55, 20, UIPaste,
            ZAY_GESTURE_T(t, this)
            {
                if(t == GT_InReleased || t == GT_OutReleased)
                {
                    const sint32 NewRootID = LoadTreeFromClipboard();
                    if(NewRootID != -1)
                    if(auto NewRootBox = TOP().Access(NewRootID))
                    {
                        double PasteX = 0;
                        double PasteY = 0;
                        const sint32 LowestChildID = FindLowestDirectChild();
                        if(LowestChildID == -1)
                        {
                            PasteX = mPosX + mBodySize.w + mAddW + 200;
                            PasteY = mPosY;
                        }
                        else if(auto LowestChildBox = TOP().Access(LowestChildID))
                        {
                            const Rect LowestRect = LowestChildBox->ConstValue().GetRect();
                            PasteX = LowestRect.l;
                            PasteY = LowestRect.b + 100;
                        }
                        NewRootBox->Value().MoveTreeTo(PasteX, PasteY);
                        if(GetChildType() == ChildType::Inner && NewRootBox->ConstValue().mCompType != "gate")
                        {
                            AddChild(NewRootBox->Value(), 0);
                            Sort();
                        }
                    }
                }
            })
        ZAY_MOVE_IF(panel, 1, 1, IsPastePressed)
        {
            ZAY_RGBA(panel, 128, 128, 128, (IsPasteFocusing)? 128 : 80)
                panel.ninepatch(R("box_slot_inner_select"));
            ZAY_FONT(panel, 1.0, gTitleFont)
...
void ZEZayBox::RenderEditorDropArea(ZayPanel& panel, chars uiname, InputType type, sint32 idx, bool blanked)
{
    ...
    ZAY_RGBA_IF(panel, 0, 128, 255, 80, !blanked && !gCtrlPressing)
    ZAY_RGBA_IF(panel, 255, 128, 255, 80, !blanked && gCtrlPressing)
    ZAY_RGBA_IF(panel, 128, 128, 128, 160, IsHovered)
    {
        ...
        ZAY_RGBA(panel, 0, 0, 0, 96)
        {
            // 타이틀
            ZAY_FONT(panel, 1.0, gTitleFont)
                panel.text((blanked)? "SWAP " : ((gCtrlPressing)? "COPY      " : "MOVE      "),
                    UIFA_RightMiddle, UIFE_Left);
            // 앞쪽에 삽입된다는 표식
...
void ZEZayBox::RenderEditorDragCell(ZayPanel& panel, InputType type, sint32 idx)
{
    ...
    const Point WireBegin(
        panel.w() / 2 - mDraggingEditorPos[type][idx].x,
        panel.h() / 2 - mDraggingEditorPos[type][idx].y);
...
{
    hook(mDraggingEditorPos[type].AtWherever(idx))
    {
        fish.x += addx;
        fish.y += addy;
    }
...
void ZEZayBox::EditorDrop(bool swap, InputType type, sint32 drop_idx, ZEZayBox& drag, sint32 drag_idx)
{
    if(&drag != this)
    {
        if(swap)
        {
            const Strings DropValues = SubInput(type, drop_idx, false);
            const Strings DragValues = drag.SubInput(type, drag_idx, false);
            AddInput(type, drop_idx, DragValues);
            drag.AddInput(type, drag_idx, DropValues);
        }
        else
        {
            const Strings DragValues = drag.SubInput(type, drag_idx, gCtrlPressing);
            AddInput(type, drop_idx, DragValues);
        }
    }
    else if(drop_idx != drag_idx)
    {
        if(swap)
        {
            ...
            const Strings MaxValues = SubInput(type, MaxIdx, false);
            const Strings MinValues = SubInput(type, MinIdx, false);
            AddInput(type, MinIdx, MaxValues);
            AddInput(type, MaxIdx, MinValues);
        }
        else
        {
            const Strings DragValues = SubInput(type, drag_idx, gCtrlPressing);
...
sint32 ZEZayBox::Copy()
{
    ...
    // 드래그중인 에디터위치도 정리
    for(sint32 i = 0; i < IT_Max; ++i)
        mDraggingEditorPos[i].Clear();
...
sint32 ZEZayBox::LoadTreeFromClipboard()
{
    const String JsonText = Platform::Utility::RecvFromTextClipboard();
    ...
    auto NewRootObject = ZEZayBox::CREATOR()(Json("compname").GetText());
    auto& NewRoot = NewRootObject.Value();
    NewRoot.mExpanded = (Json("expanded").GetInt(1) != 0);
    NewRoot.mPosX = Json("posx").GetInt(0);
    NewRoot.mPosY = Json("posy").GetInt(0);
    NewRoot.mAddW = Json("addw").GetInt(0);
    NewRoot.mParent = -1;
    NewRoot.mDebugOrder = -1;
    NewRoot.mHooked = false;
    NewRoot.mHookPos = Point(-BallX, 0);
    NewRoot.ReadJson(Json);

    const sint32 NewRootID = NewRoot.mID;
    ZEZayBox::TOP()[NewRootID] = NewRootObject;
    NewRoot.Load(NewRoot.children(), Json("ui"), &NewRoot, 0);
    NewRoot.ClearCompID();
    return NewRootID;
}

sint32 ZEZayBox::FindLowestDirectChild()
{
    sint32 Result = -1;
    sint32 MaxBottom = -0x7FFFFFFF;
    if(auto CurChildren = GetChildrenGroup(0))
    for(sint32 i = 0, iend = CurChildren->Count(); i < iend; ++i)
        if(auto CurBox = TOP().Access((*CurChildren)[i]))
        {
            const Rect CurRect = CurBox->ConstValue().GetRect();
            if(MaxBottom < CurRect.b)
            {
                MaxBottom = CurRect.b;
                Result = (*CurChildren)[i];
            }
        }
    return Result;
}

void ZEZayBox::MoveTreeTo(double x, double y)
{
...
void ZEZayBox::BodyParamGroup::RenderParamGroup(ZayPanel& panel)
{
    ...
    ZAY_INNER(panel, 1)
    {
        ...
        ZAY_INNER(panel, 3)
        {
            ...
            for(sint32 i = 0, iend = mParams.Count(); i <= iend; ++i)
            {
                const bool IsDragged = (i < mBox.mDraggingEditorPos[0].Count() && mBox.mDraggingEditorPos[0][i] != Point());
                const Point& DragAdd = (drag == 1 && IsDragged)? mBox.mDraggingEditorPos[0][i] : Point();
    ...
    ZAY_INNER(panel, 1)
    {
        ...
        ZAY_INNER(panel, 3)
        {
            ...
            for(sint32 i = 0, iend = ((sub)? sub->mInputs.Count() : 0) + mInputs.Count(); i <= iend; ++i)
            {
                ...
                const bool IsDragged = (DragInputIdx < mBox.mDraggingEditorPos[DragInputType].Count() && mBox.mDraggingEditorPos[DragInputType][DragInputIdx] != Point());
                const Point& DragAdd = (drag == 1 && IsDragged)? mBox.mDraggingEditorPos[DragInputType][DragInputIdx] : Point();
...
void ZEZayBoxStarter::Render(ZayPanel& panel)
{
    ...
    ZAY_INNER(panel, ScissorGap)
    {
        ...
        // 타이틀
        RenderTitle(panel, mCompType, GetChildType(), false, false, true, true, false);
...
ZEZayBox::ChildType ZEZayBoxStarter::GetChildType() const
{
    return ChildType::Inner;
}
...
Strings ZEZayBoxStarter::SubInput(InputType type, sint32 idx, bool copyonly)
{
    Strings OldValues = ZEZayBox::SubInput(type, idx, copyonly);
    if(type == IT_Value)
    {
        OldValues.AtAdding() = mCreateGroup.mInputs[idx].mKey;
        OldValues.AtAdding() = mCreateGroup.mInputs[idx].mValue;
        if(!copyonly) mCreateGroup.SubValue(idx);
    }
    return OldValues;
}
...
void ZEZayBoxContent::Render(ZayPanel& panel)
{
    ...
    ZAY_XYWH(panel, sint32(mPosX), sint32(mPosY), mBodySize.w + mAddW, TitleBarHeight + ((mExpanded)? mBodySize.h : 0))
    {
        ...
        ZAY_INNER(panel, ScissorGap)
        {
            ...
            // 타이틀
            RenderTitle(panel, mCompType, GetChildType(), mHasParam, true, true, true, true);
...
ZEZayBox::ChildType ZEZayBoxContent::GetChildType() const
{
    return mChildType;
}
...
Strings ZEZayBoxContent::SubInput(InputType type, sint32 idx, bool copyonly)
{
    Strings OldValues = ZEZayBox::SubInput(type, idx, copyonly);
    switch(type)
    {
    ...
    case IT_Value:
        ...
        if(!copyonly) mSetGroup.SubValue(idx);
        break;
    }
    return OldValues;
}
...
void ZEZayBoxLayout::Render(ZayPanel& panel)
{
    ...
    ZAY_XYWH(panel, sint32(mPosX), sint32(mPosY), mBodySize.w + mAddW, TitleBarHeight + ((mExpanded)? mBodySize.h : 0))
    {
        ...
        {
            // 타이틀
            RenderTitle(panel, mCompType, GetChildType(), true, true, true, true, true);
...
ZEZayBox::ChildType ZEZayBoxLayout::GetChildType() const
{
    return ChildType::Inner;
}
...
Strings ZEZayBoxLayout::SubInput(InputType type, sint32 idx, bool copyonly)
{
    Strings OldValues = ZEZayBox::SubInput(type, idx, copyonly);
    switch(type)
    {
...
ZEZayBox::ChildType ZEZayBoxCode::GetChildType() const
{
    return ChildType::Inner;
}
...
Strings ZEZayBoxCode::SubInput(InputType type, sint32 idx, bool copyonly)
{
    Strings OldValues = ZEZayBox::SubInput(type, idx, copyonly);
    if(type == IT_Value)
    {
        ...
        if(!copyonly) mCodeGroup.SubValue(idx);
    }
    return OldValues;
}
...
void ZEZayBoxJumpOrGate::Render(ZayPanel& panel)
{
    ...
    ZAY_XYWH(panel, sint32(mPosX), sint32(mPosY), mBodySize.w + mAddW, TitleBarHeight + ((mExpanded)? mBodySize.h : 0))
    {
        ...
        {
            // 타이틀
            RenderTitle(panel, mCompType, GetChildType(), false, true, true, true, true);
...
ZEZayBox::ChildType ZEZayBoxJumpOrGate::GetChildType() const
{
    return (mIsGate)? ChildType::Inner : ChildType::None;
}
...
ZEZayBox::ChildType ZEZayBoxLoop::GetChildType() const
{
    return ChildType::Inner;
}
...
void ZEZayBoxCondition::Render(ZayPanel& panel)
{
    ...
    ZAY_XYWH(panel, sint32(mPosX), sint32(mPosY), mBodySize.w + mAddW, TitleBarHeight + ((mExpanded && mHasElseAndOperation)? mBodySize.h : 0))
    {
        ...
        ZAY_INNER(panel, ScissorGap)
        {
            // 타이틀
            if(mOperation.mWithElse)
                RenderTitle(panel, "el" + mCompType, GetChildType(), false, true, mHasElseAndOperation, true, true);
            else RenderTitle(panel, mCompType, GetChildType(), false, true, mHasElseAndOperation, true, true);
...
ZEZayBox::ChildType ZEZayBoxCondition::GetChildType() const
{
    return ChildType::None;
}
...
void ZEZayBoxError::Render(ZayPanel& panel)
{
    ...
    ZAY_XYWH(panel, sint32(mPosX), sint32(mPosY), mBodySize.w + mAddW, TitleBarHeight)
    {
        ...
        ZAY_INNER_SCISSOR(panel, -ScissorGap)
        ZAY_INNER(panel, ScissorGap)
            RenderTitle(panel, mCompType, GetChildType(), false, true, false, true, true);
    }
}
...
ZEZayBox::ChildType ZEZayBoxError::GetChildType() const
{
    return ChildType::Inner;
}
...
Strings ZEZayBoxError::SubInput(InputType type, sint32 idx, bool copyonly)
{
    Strings OldValues = ZEZayBox::SubInput(type, idx, copyonly);
    switch(type)
    {
